...
public class ProjectTest {
	...
	public void testOnlyOneOccupiedTile() {
		...
		for (int i = 0; i < 121; i++) {
			if (project.getBoard().getHexagon(i).getHexagonStateContext().getCurrentState().getClass().equals(OccupiedTile.class)) {
				tileList.add(i);
			}
		}
	...
	public void testNewTurn() {
		...
		for (int i = 0; i < 121; i++) {
			if (project.getBoard().getHexagon(i).getHexagonStateContext().getCurrentState().getClass().equals(OccupiedTile.class)) {
				initialOccupiedTileIndex = project.getBoard().getHexagon(i).getIndex();
			}
		}
		project.NewTurn();
		for (int i = 0; i < 121; i++) {
			if (project.getBoard().getHexagon(i).getHexagonStateContext().getCurrentState().getClass().equals(OccupiedTile.class)) {
				nextOccupiedTileIndex = project.getBoard().getHexagon(i).getIndex();
			}
		}
